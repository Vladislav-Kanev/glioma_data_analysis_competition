...
def select_by_correlation_value(correlation_estimator: FeatureCorrelation,
                                min_score: float = 0.5) -> dict[str, float]:
...
def get_feature_estimator(training_data: pd.DataFrame, targets: pd.DataFrame,
                          method: Literal['mutual_info-classification', 'pearson']) -> FeatureCorrelation:
